...
@Injectable()
export class usuariosTablesService {
  public url: string;
  ...
  editProducto(token, id: string, inventario) {
      ...
      return this._http.put(this.url + 'update-user/'+id, inventario, {headers: headers})
          .map(res => res.json());
  }
  ...
  deleteProducto(token, id: string) {
    console.log('id');
      ...
      let options = new RequestOptions({headers: headers});
      return this._http.delete(this.url + 'delete-user/'+id, options)
          .map(res => res.json());
  }
